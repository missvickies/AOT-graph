function draw() {
  ...
  */
  var nodes = [
    ...
    { id: 42, label: "Mikasa's Mother", group: 6, opacity: 0.5 },
    { id: 43, label: "Ackermann", group: 6, opacity: 0.5 },
    { id: 44, label: "Titans", group: 9, shape: 'box'},
...
for (let i = 0; i < nodes.length ; i++) {
  ...
  if ( neg.includes(nodes[i].id)){ edges.push({from: 0, to: nodes[i].id, color: "red"}) }
   ...
   //positive edges
   let pos = [];
   var matches2 = ['Levi Ackermann','Mikasa Ackermann', 'Marco Bott', 'Hange Zoë','Erwin Smith','Wall Maria','Conny Springer',
...
"Christa Lenz", "Sasha Braus", "Jean Kirschtein", "Marco Bott", "Conny Springer", "Levi Ackermann", "Erwin Smith", "Keith Sadies", 
"Hange Zoë", "Miche Zacharius", "Wall Maria", "Carla Jaeger", "Dot Pixis", "Kitz Woermann", "Rico Brzensk", "Mitabi Jarnach", 
"Ian Dietrich", "Hannes", "Dino Reeves", "Daz", "Hannah Diamant", "Franz Kefka", "Ymir", 'Mina Carolina', 'Milieus Zeremski', 'Nack Tierce', 'Thomas Wagner'];
if (~ matches.indexOf(nodes[i].label)) { neg.push(nodes[i].id)}
